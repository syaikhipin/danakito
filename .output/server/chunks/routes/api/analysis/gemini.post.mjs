...
const gemini_post = defineEventHandler(async (event) => {
  ...
  try {
    ...
    const aiResponse = (_c = (_b = (_a = data.candidates[0]) == null ? void 0 : _a.content) == null ? void 0 : _b.parts[0]) == null ? void 0 : _c.text;
    let analysisResult;
    try {
...
function generateEnhancedMockResponse(requestData) {
...
function generateBankCreditOptions(amount, tenor) {
  const banks = [
    ...
    { name: "Capital One Spark", baseRate: 8, fee: 0.01, requirements: ["Fair credit (670+)", "No minimum revenue"], pros: ["Simple application", "Good for new businesses"], cons: ["Higher rates for lower credit scores", "Fewer physical branches"] }
  ];
...
export { gemini_post as default };
//# sourceMappingURL=gemini.post.mjs.map
